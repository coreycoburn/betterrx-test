...
class NPIRegistryClientFake implements NPIInterface
{
    ...
    {
        if (empty($terms)) {
            return $this->getUnprocessedResponse();
        }

        if ($terms === ['first_name' => 'Corey']) {
            return $this->getFirstNameResponse();
        }

        throw new Exception('Invalid NPI Mock');
    }

    private function getUnprocessedResponse() : array
    {
        return [
            'code' => 422,
    ...
    private function getFirstNameResponse(): array
    {
        return  [
            'result_count' => 10,
            'results' => [
